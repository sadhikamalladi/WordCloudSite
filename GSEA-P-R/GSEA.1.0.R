...
#   max.sets.phen2 <- length(set.table.phen2[,1])
   ...
   rownames(Itable) <- set.table.phen1[1:max.sets.phen1, 2]
   ...
   rownames(D.phen1) <- set.table.phen1[1:max.sets.phen1, 2]
   ...
   GSEA.HeatMapPlot2(V = data.matrix(D.phen1), row.names = rownames(D.phen1), col.names = names(D.phen1), main = "Leading Subsets Assignment", sub = paste(doc.string, " - ", phen1, sep=""), xlab=" ", ylab=" ", color.map = cmap) 
   ...
   rownames(DT2.phen1) <- all.leading.genes
#   GSEA.write.gct(DT2.phen1, filename=outputfile2.phen1)
   ...
   rownames(Itable) <- set.table.phen2[1:max.sets.phen2, 2]
   ...
   rownames(D.phen2) <- set.table.phen2[1:max.sets.phen2, 2]
   ...
   cmap <-  c("#AAAAFF", "#111166")
   GSEA.HeatMapPlot2(V = data.matrix(D.phen2), row.names = rownames(D.phen2), col.names = names(D.phen2), main = "Leading Subsets Assignment", sub = paste(doc.string, " - ", phen2, sep=""), xlab=" ", ylab=" ", color.map = cmap) 
